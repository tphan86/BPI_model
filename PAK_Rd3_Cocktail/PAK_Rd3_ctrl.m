...
data_table = readtable('dataset_PAK_Rd3.xlsx', 'Sheet', 'PAK_Rd3_Grouped');
...
S0 = 1.03e5;   % can be estimated using S0 = S(t)/exp(r*t) after
               % estimating r using log-linear regression for the first 3 time
               % points (t = 2, 4, 6)
...
title(t, 'PAK Rd3 Control Data Fitting (No Phage)', ...
...
title('(D) Summary', 'FontSize', 12);
ax.TitleHorizontalAlignment = 'left';
box on;

% Save figure as PDF
exportgraphics(fig, 'control_fitting_PAK_Rd3.pdf', 'ContentType', 'vector', 'Resolution', 300);
fprintf('\nFigure saved: control_fitting_PAK_Rd3.pdf\n');
...
save('control_fitting_results_PAK_Rd3.mat', 'results');
fprintf('Results saved: control_fitting_results_PAK_Rd3.mat\n');
