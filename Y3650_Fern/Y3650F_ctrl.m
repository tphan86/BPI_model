%% Fit Control Data for P. larvae Y-3650
...
data_table = readtable('dataset_Y3650F.xlsx', 'Sheet', 'Y3650_Fern_Grouped');
...
S0 = 4.2e6;                 % Initial bacterial density (CFU/mL) from manuscript

% Note: Initial OD600 ≈ 0.009 corresponds to 9E6 CFU/mL
% This is close to the reported 4.2E6 CFU/mL
% We can either use the reported value or the OD-derived value
% Here we use the OD-derived value for consistency with the data
S0_from_OD = od_mean(1) * OD_to_CFU;
fprintf('Initial S0 from manuscript: %.2e CFU/mL\n', S0);
fprintf('Initial S0 from OD600: %.2e CFU/mL\n', S0_from_OD);
fprintf('Using S0 from OD600 for fitting consistency.\n\n');
S0 = S0_from_OD;
...
title(t, 'P. larvae Y-3650 Control Data Fitting (No Phage)', ...
    'FontSize', 18, 'FontWeight', 'bold');

% Panel A: CFU/mL scale
nexttile;
ax = gca;
errorbar(time_hr, S_data, S_std, 'ro', 'MarkerSize', 4, 'MarkerFaceColor', 'r', ...
    'DisplayName', 'Control data (mean \pm std)', 'LineWidth', 0.5);
hold on;
...
nexttile;
ax = gca;
plot(time_hr, residuals / OD_to_CFU, 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');
hold on;
...
% Save figure as PDF
exportgraphics(fig, 'control_fitting_Y3650.pdf', 'ContentType', 'vector', 'Resolution', 300);
fprintf('\nFigure saved: control_fitting_Y3650.pdf\n');
...
save('control_fitting_results_Y3650.mat', 'results');
fprintf('Results saved: control_fitting_results_Y3650.mat\n');
