...
fprintf('  Data range: %.2e to %.2e CFU/mL', min(data_high_mean), max(data_high_mean)); %[output:80f01d8f]
...
% Initial values
r = 1.1478;      % growth rate of susceptible bacteria S
a = 468.535;      % interspecific competition rate between S and R
a_ss = 89.7757;  % intraspecific competition rate of S
a_rr = 12.6446;   % intraspecific competition rate of R
delta = 1e-3;    % rate of resistance acquisition
gamma = 0.3304;  % fitness cost of phage resistance
phi = 5.9373e-8;  % adsorption rate of phage P
beta = 10.94;       % burst size of phage P
m = 5.3187e-2;      % decay rate of phage P
S0 = 66084;     % initial value of S
R0 = 1027;        % initial value of R
P0 = 356;      % initial value of P
...
func = @(para_fit) robust_objective_function(para_fit, data_high_mean, time);
...
ss_res = sum((data_high_mean - model_at_data_points).^2);
ss_tot = sum((data_high_mean - mean(data_high_mean)).^2);
r_squared = 1 - ss_res/ss_tot;

fprintf('\n=== Model Quality Metrics ===');
fprintf('R-squared: %.4f\n', r_squared);
fprintf('RMSE: %.2e\n', sqrt(mean((data_high_mean - model_at_data_points).^2)));
fprintf('Mean Absolute Error: %.2e\n', mean(abs(data_high_mean - model_at_data_points)));
...
if exist('data_high_std', 'var') && ~isempty(data_high_std)
    errorbar(time, data_high_mean, data_high_std, '.', 'MarkerSize', 25, ...
        'MarkerEdgeColor', 'red', 'Color', 'red', 'LineWidth', 1.5);
else
    plot(time, data_high_mean, '.', 'MarkerSize', 25, ...
...
title('Y3650 Fern - High Group', 'FontSize', 18);
...
print('Y3650_Fern_high', '-dpdf')

% Optional: Save fitted parameters
fitted_Y3650F_high = fitted_para;
save('fitted_param_Y3650F_high.mat', 'fitted_Y3650F_high', 'r_squared', 'final_rmse', 'optimization_time');

fprintf('\nOptimization complete! Results saved to Y3650_Fern_high.pdf\n');
