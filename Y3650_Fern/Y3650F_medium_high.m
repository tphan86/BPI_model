...
fprintf('  Data range: %.2e to %.2e CFU/mL', min(data_medium_high_mean), max(data_medium_high_mean)); %[output:80f01d8f]
...
% Initial values
r = 0.7789;      % growth rate of susceptible bacteria S
a = 2.6025e-1;      % interspecific competition rate between S and R
a_ss = 56.4159;  % intraspecific competition rate of S
a_rr = 1.0233e3;   % intraspecific competition rate of R
delta = 8.1689e-4;    % rate of resistance acquisition
gamma = 0.9956;  % fitness cost of phage resistance
phi = 2e-8;  % adsorption rate of phage P
beta = 13.43;       % burst size of phage P
m = 2.710e-3;      % decay rate of phage P
S0 = 403831;     % initial value of S
R0 = 134;        % initial value of R
P0 = 72.96;      % initial value of P
...
func = @(para_fit) robust_objective_function(para_fit, data_medium_high_mean, time);
...
ss_res = sum((data_medium_high_mean - model_at_data_points).^2);
ss_tot = sum((data_medium_high_mean - mean(data_medium_high_mean)).^2);
r_squared = 1 - ss_res/ss_tot;

fprintf('\n=== Model Quality Metrics ===');
fprintf('R-squared: %.4f\n', r_squared);
fprintf('RMSE: %.2e\n', sqrt(mean((data_medium_high_mean - model_at_data_points).^2)));
fprintf('Mean Absolute Error: %.2e\n', mean(abs(data_medium_high_mean - model_at_data_points)));
...
if exist('data_medium_high_std', 'var') && ~isempty(data_medium_high_std)
    errorbar(time, data_medium_high_mean, data_medium_high_std, '.', 'MarkerSize', 25, ...
        'MarkerEdgeColor', 'red', 'Color', 'red', 'LineWidth', 1.5);
else
    plot(time, data_medium_high_mean, '.', 'MarkerSize', 25, ...
...
title('Y3650 Fern - Medium High Group', 'FontSize', 18);
...
print('Y3650_Fern_medium_high', '-dpdf')

% Optional: Save fitted parameters
fitted_Y3650F_medium_high = fitted_para;
save('fitted_param_Y3650F_medium_high.mat', 'fitted_Y3650F_medium_high', 'r_squared', 'final_rmse', 'optimization_time');

fprintf('\nOptimization complete! Results saved to Y3650_Fern_medium_high.pdf\n');
