...
P0_values = [0.347, 3.47, 34.7, 347];  % Low, Medium, Med-High, High
...
fprintf('=== Fitting ALL 4 Treatments (Including Med-High) ===\n');
...
eps_init = [0.0324, 0.0066, 0.005, 1e-4];       % Decreasing with MOI
gamma_init = [1.0, 0.6110, 0.4, 0.4179];          % Fitness cost
delta_init = [1e-3, 5.76e-4, 1e-4, 1e-3];         % Resistance acquisition
phi_s_init = [1e-9, 1.8e-9, 1e-8, 4.99e-7];       % Adsorption rate
R0_init = [1e6, 2.23e4, 1e5, 1e6];                % Initial resistant
...
lb_eps = 0.0001 * ones(4, 1);   ub_eps = 0.5 * ones(4, 1);
lb_gamma = 0.0001 * ones(4, 1);   ub_gamma = 1.0 * ones(4, 1);
...
title(t_layout, 'Y3650 Fern: Global Fitting Results (All 4 Treatments)', ...
...
for i = 1:n_treatments
    ...
    title(sprintf('%s (P_0=%.2f)\nR^2=%.3f', ...
        treatment_names{i}, P0_values(i), R_squared_all(i)), 'FontSize', 11);
...
exportgraphics(fig1, 'Y3650_Fern_4treatment_fit_5shared.pdf', 'ContentType', 'vector', 'Resolution', 300);
...
title(t_layout2, 'Y3650 Fern: Treatment-Specific Parameters vs MOI (All 4 Treatments)', ...
    'FontSize', 14, 'FontWeight', 'bold');
...
exportgraphics(fig2, 'Y3650_Fern_4treatment_params_5shared.pdf', 'ContentType', 'vector', 'Resolution', 300);
...
save('fitting_results_4treatments_Y3650F_5shared.mat', 'results');
