...
fprintf('  Data range: %.2e to %.2e CFU/mL', min(data_medium_mean), max(data_medium_mean)); %[output:80f01d8f]
...
% Initial values
r = 1.5531;      % growth rate of susceptible bacteria S
a = 1.5166e-7;      % interspecific competition rate between S and R
a_ss = 42.0266;  % intraspecific competition rate of S
a_rr = 9.0342;   % intraspecific competition rate of R
delta = 1e-3;    % rate of resistance acquisition
gamma = 0.1751;  % fitness cost of phage resistance
phi = 2e-8;  % adsorption rate of phage P
beta = 10.68;       % burst size of phage P
m = 9.5569e-2;      % decay rate of phage P
S0 = 2303;     % initial value of S
R0 = 1184;        % initial value of R
P0 = 3.485;      % initial value of P
...
func = @(para_fit) robust_objective_function(para_fit, data_medium_mean, time);
...
ss_res = sum((data_medium_mean - model_at_data_points).^2);
ss_tot = sum((data_medium_mean - mean(data_medium_mean)).^2);
r_squared = 1 - ss_res/ss_tot;

fprintf('\n=== Model Quality Metrics ===');
fprintf('R-squared: %.4f\n', r_squared);
fprintf('RMSE: %.2e\n', sqrt(mean((data_medium_mean - model_at_data_points).^2)));
fprintf('Mean Absolute Error: %.2e\n', mean(abs(data_medium_mean - model_at_data_points)));
...
if exist('data_medium_std', 'var') && ~isempty(data_medium_std)
    errorbar(time, data_medium_mean, data_medium_std, '.', 'MarkerSize', 25, ...
        'MarkerEdgeColor', 'red', 'Color', 'red', 'LineWidth', 1.5);
else
    plot(time, data_medium_mean, '.', 'MarkerSize', 25, ...
...
title('Y3650 Fern - Medium Group', 'FontSize', 18);
...
print('Y3650_Fern_medium', '-dpdf')

% Optional: Save fitted parameters
fitted_Y3650F_medium = fitted_para;
save('fitted_param_Y3650F_medium.mat', 'fitted_Y3650F_medium', 'r_squared', 'final_rmse', 'optimization_time');

fprintf('\nOptimization complete! Results saved to Y3650_Fern_medium.pdf\n');
