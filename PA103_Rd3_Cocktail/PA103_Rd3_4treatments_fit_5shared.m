...
% (Low, Medium, Med-High, High) to test if global fitting can capture
% all dynamics including Med-High.
% wi
...
excelFileName = 'dataset_PA103_Rd3.xlsx';
sheetName = 'PA103_Rd3_Grouped';

data_table = readtable(excelFileName, 'Sheet', sheetName);
time = data_table.Time;  % hours

OD_to_CFU = 1e9;

% Load ALL 4 treatments (including Med-High)
...
% P0 values for all 4 treatments
P0_values = [0.1, 1, 10, 100];  % Low, Medium, Med-High, High
...
fprintf('=== Fitting ALL 4 Treatments ===\n');
fprintf('Treatments: Low (P0=0.1), Medium (P0=1), Med-High (P0=10), High (P0=100)\n');
...
% TREATMENT-SPECIFIC (4 treatments: Low, Med, MedHigh, High)
eps_init = [0.0097, 0.0517, 0.0063, 0.0032];       % Decreasing with MOI
gamma_init = [0.6647, 0.4306, 0.5102, 2.7249e-4];          % Fitness cost
delta_init = [3.9212e-5, 3.872e-4, 1e-6, 5.6098e-4];         % Resistance acquisition
phi_s_init = [4.4914e-9, 4.1255e-9, 6.2647e-8, 2.3656e-7];       % Adsorption rate
R0_init = [5.676e6, 6.0949e6, 7.7852e4, 2.2168e6];                % Initial resistant

params_init = [a_rs_init; a_sr_init; a_rr_init; beta_init; m_init; ...
               eps_init(:); gamma_init(:); delta_init(:); phi_s_init(:); R0_init(:)];

% Bounds
lb_shared = [1e-2; 1e-2; 1e-2; 10; 1.5e-3];
ub_shared = [500; 500; 100; 100; 0.5];
...
k_params = 25;  % 5 shared + 20 treatment-specific
...
fprintf('\n%s\n', repmat('=', 1, 70));
fprintf('FITTING RESULTS (All 4 Treatments)\n');
fprintf('%s\n', repmat('=', 1, 70));

fprintf('\n--- SHARED Parameters ---\n');
fprintf('  a_rs = %.4f, a_sr = %.4f, a_rr = %.4f\n', a_rs_fit, a_sr_fit, a_rr_fit);
fprintf('  beta = %.2f, m = %.4f\n', beta_fit, m_fit);

fprintf('\n--- TREATMENT-SPECIFIC Parameters ---\n');
fprintf('%-10s %10s %10s %10s %12s %12s %12s\n', ...
    'Treatment', 'P0', 'epsilon', 'gamma', 'delta', 'phi_s', 'R0');
fprintf('%s\n', repmat('-', 1, 80));
for i = 1:n_treatments
    fprintf('%-10s %10.2f %10.4f %10.4f %12.2e %12.2e %12.2e\n', ...
        treatment_names{i}, P0_values(i), eps_fit(i), gamma_fit(i), ...
        delta_fit(i), phi_s_fit(i), R0_fit(i));
end
...
fig1 = figure('Units', 'inches', 'Position', [1, 1, 14, 10], ...
    'PaperUnits', 'inches', 'PaperSize', [14, 10], 'PaperPosition', [0, 0, 14, 10]);

t_layout = tiledlayout(2, 3, 'TileSpacing', 'compact', 'Padding', 'compact');
title(t_layout, 'PA103 Rd Cocktail: Global Fitting Results with M1 (5 shared parameters)', ...
    'FontSize', 18, 'FontWeight', 'bold');

colors = lines(4);  % 4 colors for 4 treatments
tspan_plot = linspace(2, max(time), 500);

% Plot all 4 fitted treatments
for i = 1:n_treatments
    nexttile;
    
    [~, S_sim, R_sim, ~] = simulate_model(params_opt, tspan_plot, ...
        P0_values(i), i, r_fixed, a_ss_fixed, S0_fixed, K);
    total_sim = S_sim + R_sim;
    
    errorbar(time, all_data_mean{i}, all_data_std{i}, 'o', ...
        'MarkerSize', 5, 'Color', colors(i,:), 'MarkerFaceColor', colors(i,:), ...
        'LineWidth', 1, 'CapSize', 3);
    hold on;
    plot(tspan_plot, total_sim, 'k-', 'LineWidth', 3);
    plot(tspan_plot, S_sim, 'b--', 'LineWidth', 1.5);
    plot(tspan_plot, R_sim, 'r:', 'LineWidth', 1.5);
    
    xlabel('Time (hours)', 'FontSize', 14);
    ylabel('CFU/mL', 'FontSize', 14);
    title(sprintf('%s (P_0=%.2f)\nR^2=%.3f', ...
        treatment_names{i}, P0_values(i), R_squared_all(i)), 'FontSize', 16);
    
    if i == 1
        legend('Data', 'S+R', 'S', 'R', 'Location', 'best', 'FontSize', 12);
    end
...
nexttile;
plot(time, data_low_mean, 'b-', 'LineWidth', 2.5, 'DisplayName', 'Low');
hold on;
plot(time, data_med_mean, 'g-', 'LineWidth', 2.5, 'DisplayName', 'Medium');
plot(time, data_medhigh_mean, 'r-', 'LineWidth', 2.5, 'DisplayName', 'Med-High');
plot(time, data_high_mean, 'm-', 'LineWidth', 2.5, 'DisplayName', 'High');
xlabel('Time (hours)', 'FontSize', 14);
ylabel('CFU/mL', 'FontSize', 14);
title('All Treatments Comparison', 'FontSize', 16);
...
summary_text = {
    ...
    '', ...
    '\bf{Shared:}', ...
    sprintf('  a_{rs}=%.2f, a_{sr}=%.2f', a_rs_fit, a_sr_fit), ...
...
exportgraphics(fig1, 'PA103_Rd3_4treatment_fit_5shared.pdf', 'ContentType', 'vector', 'Resolution', 300);
...
title(t_layout2, 'PA103 Rd3 Cocktail: Treatment-Specific Parameters vs MOI (All 4 Treatments)', ...
...
exportgraphics(fig2, 'PA103_Rd3_4treatment_params_5shared.pdf', 'ContentType', 'vector', 'Resolution', 300);
...
results.AIC = AIC;
results.BIC = BIC;
results.note = 'All 4 treatments fitted globally';

save('fitting_results_4treatments_PA103_Rd3_5shared.mat', 'results');
...
fprintf('%s\n', repmat('=', 1, 85));
fprintf('\nTable X: Treatment-specific parameters (Y-3650 + Fern) - All 4 Treatments\n\n');
fprintf('%-10s | %8s | %8s | %8s | %10s | %10s | %10s | %6s\n', ...
    'Treatment', 'P0', 'epsilon', 'gamma', 'delta', 'phi_s', 'R0', 'R^2');
fprintf('%s\n', repmat('-', 1, 85));
for i = 1:n_treatments
    fprintf('%-10s | %8.2f | %8.4f | %8.4f | %10.2e | %10.2e | %10.2e | %6.3f\n', ...
        treatment_names{i}, P0_values(i), eps_fit(i), gamma_fit(i), ...
...
function [t_out, S_out, R_out, P_out] = simulate_model(params, time, P0, ...
    ...
    % params(1:5) = [a_rs, a_sr, a_rr, beta, m]
    ...
    epsilon = params(5 + treatment_idx);
    gamma = params(9 + treatment_idx);
    delta = params(13 + treatment_idx);
    phi_s = params(17 + treatment_idx);
    R0 = params(21 + treatment_idx);
